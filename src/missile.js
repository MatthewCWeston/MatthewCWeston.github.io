...
export class Missile {
  ...
  constructor(pos, vel) {
    ...
    this.prevPos = [pos[0], pos[1]];     // for trails
  }

  /** Returns [hitShipIdx, removeSelf].  hitShipIdx === -1 if no ship hit. */
  update(ships, speed) {
    ...
    for (let si = 0; si < ships.length; si++) {
      const s = ships[si];
      // Ships in hyperspace can't be hit.
      if (s.h_reload > HYPERSPACE_RECHARGE - HYPERSPACE_REENTRY) continue;
      const dx = this.pos[0] - s.pos[0];
      const dy = this.pos[1] - s.pos[1];
      if (Math.hypot(dx, dy) < s.size) return [si, true];
    }
    if (Math.hypot(this.pos[0], this.pos[1]) < STAR_SIZE) return [-1, true];

    wrap(this.pos);
    this.life -= 1 * speed;
    return [-1, this.life <= 0];
  }
